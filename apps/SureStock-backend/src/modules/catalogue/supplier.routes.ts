...
export default async function supplierRoutes(app: FastifyInstance) {
  ...
  app.delete(
    ...
    async (request, reply) => {
      const { id } = supplierIdParamsSchema.parse(request.params);
      await deleteSupplier(app.prisma, id);
